...
- Agent 完成一个编译通过、自洽的改动单元后，可以主动 commit 并推送，不必等用户要求；半成品、临时插桩、未验证的中间态不提交也不推送。推送前遵守下一条的 fetch / rebase 规则。提交前用 `git status` 确认改动范围：发现并行会话进行中的改动，只提交自己名下的文件，不要把别人的工作扫进自己的提交。
