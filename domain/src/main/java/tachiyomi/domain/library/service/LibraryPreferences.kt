...
class LibraryPreferences(
    ...
    // Cloud sources have their own sort default (name descending so the newest chapter
    // shows first), independent from the local library's default.
    val sortCloudChapterBySourceOrNumber: Preference<Long> = preferenceStore.getLong(
        "default_cloud_chapter_sort_by_source_or_number",
        Manga.CHAPTER_SORTING_ALPHABET,
    )

    val sortCloudChapterByAscendingOrDescending: Preference<Long> = preferenceStore.getLong(
        "default_cloud_chapter_sort_by_ascending_or_descending",
        Manga.CHAPTER_SORT_DESC,
    )
    ...
    fun setChapterSettingsDefault(manga: Manga) {
        ...
        val sorting = if (manga.sorting == Manga.CHAPTER_SORTING_CUSTOM) {
            Manga.CHAPTER_SORTING_ALPHABET
        } else {
            manga.sorting
        }
        val direction = if (manga.sortDescending()) Manga.CHAPTER_SORT_DESC else Manga.CHAPTER_SORT_ASC
        if (manga.source == 0L) {
            sortChapterBySourceOrNumber.set(sorting)
            sortChapterByAscendingOrDescending.set(direction)
        } else {
