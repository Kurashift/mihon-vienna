...
private fun TransitionText(
    ...
    Column(
        ...
        if (topChapter != null) {
            ...
            Spacer(Modifier.height(if (compact) CompactVerticalSpacerSize else VerticalSpacerSize))
        } else {
...
private fun ChapterGapWarning(
    ...
    val text = pluralStringResource(MR.plurals.missing_chapters_warning, count = gapCount, gapCount)
    if (compact) {
        Text(
            text = text,
            modifier = modifier.secondaryItemAlpha(),
...
private val VerticalSpacerSize = 8.dp
private val CompactVerticalSpacerSize = 2.dp
