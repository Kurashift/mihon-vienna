...
import eu.kanade.tachiyomi.ui.audio.AudioDetailScreen
...
class MangaScreen(
    ...
    override fun Content() {
        ...
        val onRandomClicked: (() -> Unit)? = if (successState.manga.isLocal()) {
            {
                if (!randomInProgress) {
        ...
        // a pick is in flight so the transition only happens once. The currently
        // open manga is excluded so the button never randomizes into itself.
        var randomGoodDoujinInProgress by remember { mutableStateOf(false) }
        val onRandomGoodDoujinClicked: (() -> Unit)? = if (successState.manga.isLocal()) {
            {
        ...
        MangaScreen(
            ...
            onClickRandomGoodDoujin = onRandomGoodDoujinClicked,
            onAudioClicked = { showAudioSheet = true }.takeIf { successState.manga.isLocal() },
            onFilterButtonClicked = viewModel::showSettingsDialog,
            onRefresh = viewModel::fetchAllFromSource,
            onContinueReading = { scope.launch { continueReading(context, viewModel.getNextUnreadChapter()) } },
            onSearch = { query, global -> scope.launch { performSearch(navigator, query, global) } },
            onLocalSearch = { query -> scope.launch { performLocalSearch(navigator, query) } },
            onOpenSource = {
                viewModel.source?.let { navigator.push(BrowseSourceScreen(it.id, null)) }
            },
            onClickHome = {
    ...
    private suspend fun performLocalSearch(navigator: Navigator, query: String) {
        val previousScreen = navigator.items.getOrNull(navigator.size - 2)
        if (previousScreen is BrowseSourceScreen && previousScreen.sourceId == LocalSource.ID) {
            navigator.pop()
            previousScreen.search(query)
        } else {
            navigator.push(BrowseSourceScreen(LocalSource.ID, query))
        }
    }
