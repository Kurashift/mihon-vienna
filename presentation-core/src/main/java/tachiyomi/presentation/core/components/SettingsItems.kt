...
@Composable
fun SortItem(label: String, sortDescending: Boolean?, selected: Boolean = false, onClick: () -> Unit) {
    Row(
        modifier = Modifier
        ...
        if (selected) {
            Icon(
                imageVector = when (sortDescending) {
            ...
            // Keep every row aligned even when nothing is selected.
