...
### Changed
- Local library: the date sort is now a sort key of its own, chosen from the menu, instead of one whose meaning followed the filters. It offers 最近在看 under 在看, 看完时间 under 看完, 标记时间 under 标记, 好本时间 under 好本子, and 导入日期 always — the dates the filter actually guarantees exist, with 导入日期 trailing since a list narrowed by an event is usually about that event. So 好本子 can be ordered by import date, and 剩余 is never offered a finish time it cannot have
- Local library: 标记 and 好本子 keep their own dates. They are two marks on two different stores — a work can be flagged without being a good doujin — so ordering a good-doujin list by flag times would have read a store that list was never filtered against
- Local library: 在看 (formerly 在读) is now ordered by when the reader last opened a chapter, not by when they last read one far enough to count. The old rule required finishing a chapter or reading past its second page, so a work the reader dipped into and left kept its old position — only works already read to the end ever moved. Any visit to the reader now moves the work to the top
- Local library: a date follows only within its own filter. 最近在看 follows the reading filter to 看完时间, and 标记时间 follows the mark filter to 好本时间 — but a change to the other filter leaves the date alone. Looking at 好本时间 and switching 在读 against 读完 is comparing the same marked works by the same date, so the date no longer moves out from under the reader on every switch; only the filter that supplies the date can move it. 导入日期 belongs to no filter and never follows
- Local library: every date sorts newest-first by default, including 导入日期. The import date used to share its direction with 标题 and 篇数, so it could never be newest-first while the title list was A-to-Z — and turning one turned the other. It now has a slot of its own, like every other date, and the ordering is applied to the list rather than left to the source
- Local library: when a filter change leaves the date key with no date to mean (全部 or 剩余 with no mark), the ordering falls back to the title key rather than to 导入日期. A reader who was on 最近在看 and lands on 全部 did not ask to see the library in arrival order
- Local library: the filters, the sort key, the date it means and each date's direction are now one value that changes in one write. They used to be separate, so a filter tap that also moved the date rebuilt the list twice — the first rebuild paired the new filter with the date being left, which that filter cannot order by, and the list briefly showed an order that never existed before the second rebuild replaced it. The listing is also derived in one pass instead of two chained ones, for the same reason
- Local library: the sort chip names its date in two characters (导入 / 在看 / 看完 / 标记 / 好本), the same width as 标题 and 篇数, with a calendar icon badged by the event. The sort menu's own rows stay two characters too — 标题 / 篇数 / 日期 — so that list reads as one set; only the date's second level and the chip's long press spell a date out in full
- Local library: the chosen entry in a filter or sort menu is a tinted row rather than a trailing tick. The tint fills the row, so the label and icon keep the same left edge as every other row — an inset band shifted the chosen row's content sideways and made it look misaligned against its neighbours
- Local library: the sort menu's date entry opens a second level listing the available dates, so the menu stays the size it always was instead of growing to five flat rows
- Local library: the sort direction has its own button beside the sort chip. Flipping it used to mean opening the menu and re-picking the active row — two taps and a popup for the half of the ordering that changes most often
- Local library: each sort key resumes the direction it was last left in, so arriving at a date sort from another key no longer inherits a direction that reads backwards — and the one tap that fixed it used to overwrite the direction of the list you came from. The title and chapter-count keys keep sharing one direction as before
- Local library: changing the sort now says what it left the list ordered by ("排序：日期 · 在看 · 降序"), where it used to say nothing at all. The filter notice says only what the filter did, since the sort is no longer a consequence of it
...
- Library: the selection's delete button is named "Delete downloaded chapters" and only appears when the picked works actually hold downloads. A red trash can sitting beside the shelf actions read as "take these off the shelf", which is the one thing it does not do
- Library: "Remove from shelf" is a plain button in the selection row again. The shelf picker goes back to filing works only, so picking just the default shelf files them there instead of doubling as a second way off the shelf
...
- Delete confirmations: the local-files dialog opens with a short title that stays on one line and names what is erased — "Delete collection files?" or "Delete chapter files?" — while the count or the work's name sits in the body
...
- Local library: the listing is now served complete rather than a page at a time, so a reorder or a filter change no longer rebuilds the pager and walks the pages again. The import date is no longer fixed to the direction the title list was left in, and a shelf change no longer re-dates a local work: putting a work on the shelf or taking it off leaves its import date alone, so it stops jumping into today's section the moment it is shelved
- Local library: importing chapters into an existing work now re-dates it to that import, so under 导入日期 it rises to the top with the rest of today's arrivals instead of staying at the position it entered the library at. A work that gained chapters is as new as one that just arrived, and it is the same date the whole screen already reads — the date sort, the date headers, the local search order and the shelf's 收藏日期 — so there is still one answer for it rather than a second date used only by the listing. The timestamp is taken when the import starts, because the updates view shows a chapter only while its fetch time is newer than the work's date: stamping the finish would hide the very chapters just added. An import that brought nothing (every candidate already present) leaves the date alone, and shelving still does not touch it

### Fixed
- Local library: changing the sort is instant again after chapters have been imported into an existing work. The source's "listing invalidated" flag was raised by every import, and the read that followed re-confirmed the cache against the base directory and returned it — but left the flag raised. Every later read then skipped the cheap age-only path and paid a full directory walk to reach the same answer, so each sort change (which re-derives the listing) took seconds to redraw. The flag is now cleared the moment the cache is re-confirmed, which is safe because an added, removed or renamed manga folder necessarily changes the base directory's own timestamp — that case still rebuilds, and the rebuild clears the flag itself. An import into an existing work only touches a subdirectory, which is why the flag used to stick: importing a new collection changed the base directory and cleared it by accident
- Local library: an import now shows up the moment it finishes, and a deletion disappears the moment it happens. Both left the screen stale until the reader tapped a filter or pulled to refresh, because the only signal either produced was the source's "listing invalidated" flag — and a flag has no observer. The import runs in a WorkManager worker that cannot rebuild a list itself, and the delete path relied on the pager's own refresh, which is a no-op for a listing served whole (`PagingData.from` carries no ui receiver, so `refresh()` does nothing). The flag is now a revision the screen collects, so one real change is one rebuild
- Local library: a folder picked for import is now the collection, named after itself, so several folders picked together become several collections instead of being merged into one. The rule used to require the picked folder to contain further container folders (a 根目录/作者/本子 layout), which left the most ordinary case out: a folder holding the works directly has no containers, so it fell through to a manually typed name, and several such folders went into one collection. A batch that includes a file still keeps the manual target, since a file is one chapter of a collection the reader names
- Local library: the import no longer refuses a folder whose layout differs from the ones already picked. That filter existed because a batch was imported through one target, which a 根目录/作者/本子 folder and a folder holding works directly could not share — a folder now names its own collection, so the two no longer conflict
- Local library: an import no longer makes the list rebuild twice. The listing served whole recorded what it had served only when it was invalidated, which recorded the listing as it stood *before* the change: the rebuild that followed then read as another change, so returning to the tab rebuilt the whole list again for the one event — visible as the list stuttering and settling into its new order late
- Random: a small pool no longer rotates through the same handful of works in a fixed order. The anti-repeat window kept `poolSize - 1` of them cooling and only stepped in once the pool ran completely dry, so it grew until exactly one work was still drawable — and a single candidate means the pick is not random at all, because the index is drawn against a list of one. With a few marks or good doujins, every hop after the first pass came out of that one survivor: the same work back every other hop. The window now holds back only the newest pick from the pool and starts a fresh round, so every hop is drawn from at least two works; a pool of two, where a hop has to be followed by the one other work, is unchanged. Larger libraries never reached this — the window holds 50 entries, so a pool bigger than that never runs dry
- Local library: the filters, the sort direction and the date choices are three controls again. The screen's control state became one value, but the decisions kept reading the per-field copies it replaced — copies that after that change were never written again, so they stayed at whatever the screen opened with. Three symptoms, one cause: picking a reading filter reset the mark filter to its startup value (so 未加入书架 could not be combined with 在看 or 看完), the sort direction button compared against a stale key (so it flipped the wrong one and the arrow appeared dead), and choosing a date was rejected by a guard that checked the stale filter pair (so the date menu did nothing). All decisions now read the one value
- Local library: a work imported through the import screen now gets a real import date. Its row was created with `date_added = 0`, which is 1970 — so it sorted as the oldest entry in the library and its date header read "unknown", while the same work discovered by browsing the folder got a real date. New imports are dated on creation, and the works already in the library are repaired once by a migration using their chapters' earliest fetch time. **This reorders the import-date sort once**, for libraries that imported rather than browsed their local works
- Local library: covers no longer flash white when a chapter is opened. The history list, the marks list and the read-review list folded the chapter's `last_modified_at` into their cover cache key, and a trigger rewrites that column on any chapter update — opening the reader, reordering, marking a good doujin — so every cover they showed was re-decoded at once. The detail screen already excluded it; all five now share one definition of the key
- Local library: marking a selection read or unread now updates the list immediately. The reading filter matches against a progress snapshot that was only reread when the screen was shown again, so a work marked unread under 看完 stayed in the list — and one marked read under 剩余 stayed out — until the reader left the screen and came back. Clearing reading history had the same lag
...
- Local library: switching the reading filter or the order now opens the list at its top and keeps it there, instead of showing the top for an instant and then landing around the end of the first page. A reload replaces the content underneath a list that is still showing the previous result set, and the list follows the card it had remembered to wherever that card sits in the replacement — fifty rows down, with no scrolling involved. A card's identity now carries the result set it came from, so a replacement's cards cannot be matched against the previous set's, while a page simply being appended still leaves the reader exactly where they were
- Local library: leaving the tab and coming back returns to where the reader left off instead of opening at the top. The list is now kept under what it shows - listing, filters and order - so coming back to the same one reuses its state and its saved position, and only a different one starts fresh
- Local library: the fast scroller no longer strands its thumb at the top after a filter change. The thumb deliberately holds its ground when the entry count grows, since that is how a landing page looks, and a filter change grows the count the same way; the thumb kept the position of the list that had just been replaced and could not be brought back by scrolling, because the track clamps it at the top. It re-anchors together with the list it belongs to now that the list itself is rebuilt per listing
